...
|WOLF_OVERRIDE_GPU_VENDOR
|_(not set)_
|Override automatic GPU vendor detection. Accepts `nvidia`, `intel`, or `amd` (case-insensitive). Useful in WSL2 environments where the virtual GPU may return an unrecognised PCI vendor ID.
|===
